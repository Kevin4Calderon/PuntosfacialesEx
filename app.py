...
def process_image(image_path):
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            for idx, landmark in enumerate(face_landmarks.landmark):
                ...
                if idx in landmarks_mapping:
                    ...
                    # Dibujar la cruz en los puntos seleccionados
    ...
    flipped_horizontal_path = os.path.splitext(image_path)[0] + '_flipped_horizontal.jpeg'
    cv2.imwrite(flipped_horizontal_path, flipped_horizontal)

    # Aumentar brillo aleatoriamente
    bright_factor = random.uniform(1.5, 2.0)
    brighter_image = np.clip(image * bright_factor, 0, 255).astype(np.uint8)
    brighter_image_path = os.path.splitext(image_path)[0] + '_brighter.jpeg'
    cv2.imwrite(brighter_image_path, brighter_image)

    # Voltear verticalmente
    flipped_vertical = cv2.flip(image, 0)
    flipped_vertical_path = os.path.splitext(image_path)[0] + '_flipped_vertical.jpeg'
    cv2.imwrite(flipped_vertical_path, flipped_vertical)

    return pd.DataFrame(facial_points_dict), output_image_path, flipped_horizontal_path, brighter_image_path, flipped_vertical_path
...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            df_facial_points, processed_image_path, flipped_horizontal_path, brighter_image_path, flipped_vertical_path = process_image(file_path)
            ...
            return render_template(
                ...
                flipped_horizontal=flipped_horizontal_path,
                brighter_image=brighter_image_path,
                flipped_vertical=flipped_vertical_path,
                csv_file='facial_points.csv'
            )
